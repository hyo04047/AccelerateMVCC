...
## 11. stage C 결과 (2026-06-20, 세션 4)

하니스 = `stage_c_bench.cpp`(Zipfian writer + OLTP point-reader + 60s LLT + Guard-safe chain 샘플러). baseline은 **standalone이라 실 InnoDB 대신 프로토타입 내 tail-only GC 모드**로 모델링(`set_gc_tail_only`: `can_pruning`을 zone 0[= global-min 아래 tail]만 보게 제한 → InnoDB purge 충실; in-middle dead zone은 deadzone 고유 능력이라 baseline에선 차단). LLT가 global-min을 pin하면 tail purge는 그 위 in-middle을 못 줄임 = vDriver Figure 12 구도. 측정 지표 = version-chain length CDF + throughput + LLT visibility oracle. (실 MySQL+sysbench 비교는 Stage D 통합 영역.)

### 11.1 방법론 (중요 — 짧은 런 함정)
- **chain length는 no-LLT에선 알고리즘이 아니라 BG-GC 스케줄링에 지배됨**: 스레드>코어(oversubscribe)면 BG GC가 굶어 deadzone publish/retire가 급감해 hot 체인이 일시 폭주(스파이크). → **controlled threading(스레드 ≤ 코어)** 필수. 16코어에서 6 writer+6 reader+1 LLT+sampler+BG GC=15.
- **deadzone(과 FG)의 신호는 LLT 시나리오 고유**: LLT 없으면 newest 아래가 전부 dead라 BG-only로도(CPU만 있으면) 체인이 짧게 유지됨 → reader/FG가 개선할 여지 없음. 따라서 모든 헤드라인은 60s LLT 하에서.
- **warm-up 제외**: GC warm-up early-return(epoch 25/50/75) 동안은 prune이 없어 체인이 솟음 → 짧은 런 percentile 오염. `warmup_ms`(기본 3s) 이전 샘플은 요약에서 제외(CSV는 전체 보존).
- **샘플러는 Guard-safe**(`chain_length_guarded`): 라이브 체인을 동시 GC/reader와 함께 안전 순회(per-traversal Guard). 기존 `chain_length`는 quiescent 전용이라 라이브 샘플링 불가였음.

### 11.2 헤드라인 (60s LLT, 6w/6r/1llt)
| 구성 | hot-chain p50 | p99 | **max** | retire(회수) | write tput | read tput |
|---|---|---|---|---|---|---|
...
- **deadzone vs tail-only**: hot-chain max **155 vs 845,977 (~5,500×)**, retire **22.4M vs 277**(LLT 하에서 tail purge는 거의 회수 불가 = 정량 확증), read tput **1.36M/s vs 487/s (~2,800×)**. tail-only는 write tput만 높음(GC를 안 해서) — write 빠르고 read 붕괴 = HTAP 함정. vDriver "extended version chain is the prime culprit"의 우리 환경 재현(InnoDB에선 undo page I/O+latch, 여기선 traversal 비용).
- **FG cooperative unlink 증분 가치**(deadzone, FG on vs off, 부하 고정·토글만): chain 분포 하향(p50 15 vs 41, p99 45 vs 84) + read tput +30%(1.36M vs 1.04M reads/s). worst-case(max)는 BG deadzone가 이미 받쳐 비슷, FG는 **read-path latency/typical chain** 개선.
- **correctness**: 전 run LLT visibility oracle OK(inconsistencies=0; deadzone는 276M searches 동안 자기 version 불변) — tail-only 폭주 baseline조차 visibility는 정확(체인이 길 뿐). 토글 경로 ASan UAF/double-free 0, TSan race 0, 20 correctness 회귀 green.

### 11.3 robustness (skew sweep, 20s, warm-up 제외)
...
deadzone는 skew 전반에서 max ~40 안정, tail-only는 어느 skew든 300k+ 폭주 → **우위 ~8,000×가 한 operating point 운빨이 아니라 견고**.

### 11.4 부수 수정 (실제 robustness 개선)
- **BG GC stop-responsiveness**: boundary catch-up for-loop가 `gc_stop_`을 안 봐서, tail-only처럼 backlog가 큰 상황에서 `stop_background_gc().join()`이 사실상 안 끝나는(무한 대기) 버그 → catch-up 매 iteration에서 stop 확인하도록 수정(종료 중 남은 drain 포기는 안전). 20 correctness 불변 확인.
