...
**구현 증분(image 중심, 작게)**: ① 고정 MPMC ring 슬롯에 상한 image 칸 추가(`UndoRec` 확장 + static_assert 갱신 + N 축소; standalone 격리 ASan/TSan) → ② populate hook이 rec 바이트를 슬롯에 캡처(상한 초과는 image 생략; 빌드·동작만, drainer는 아직 무시) → ③ drainer가 노드에 image 적재 → ④ consult가 image 반환(shadow: image vs vanilla 재구성 byte-동일 검증) → ⑤ deadzone가 image 회수(메모리 bound 측정) → ⑥ 작은/큰 BP에서 D-0 곡선 측정.
