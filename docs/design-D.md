...
- **④ (다음 세션, 읽기 연결 — 가장 큼·correctness-critical)**: 4a 가시성 `changes_visible` 정확 미러(우리 search의 max-trx_id 루프 교체) → **4b consult를 `row0vers.cc`에 SHADOW 배선(consult가 고른 image vs vanilla 실제 재구성 byte-동일=mismatch 0; churn+LLT+rollback+forced collision; 핵심 wrong-result 관문 — 적대적 검증 곁들일 것)** → 4c 캐싱 제외 게이트(off-page LOB·virtual generated col·instant-DDL·locking read 제외, committed만, full PK collision 방어, contiguity-to-head) → 4d authoritative(walk skip, image 반환).
