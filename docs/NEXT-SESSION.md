...
- **다음 = D-1b-3**: drainer의 `consume()`가 **진짜 single-consumer insert**를 하도록 — Accelerate_mvcc(+Kuku)를 innobase 빌드에 컴파일(INNOBASE_SOURCES/링크에 추가), 저수준 insert(Trx_manager/get_mutex 미사용), **ctor 사전생성 제거**(키 동적), **kuku ≥1<<16**, insert true=기록·false=실패로 **return 계약 수정**(실패 시 노드 free, silent-drop 금지), **GC off**(populate-only=메모리 무한증가, 의도). 검증 = (table,pk)별 chain 무결성·insert==drained·유계 아닌 메모리는 명시. 이후 D-1b-4(noexcept/assert 하드닝)→D-2(consult: ReadView 3-way changes_visible, hint+fallback). 안전 설계 상세 [design-D.md](design-D.md) §9.
