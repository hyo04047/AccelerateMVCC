...
- consult facade(`accel_hook`, leaf-domain): `accel_consult(table_id, pk_hash, full_pk*, pk_len,
  ...
  분류 결과(enum: HIT/MISS_ABSENT/MISS_NONCONTIG/MISS_NOVISIBLE/MISS_INELIGIBLE/MISS_GCRACE[D-5 C3 gen-gate 레이스, mode-1 한정]) 반환. HIT면
...
### 15.4 불변식 / 잔여
HIT-or-MISS 불변식 유지(HIT=vanilla byte-동일, 아니면 MISS=full walk).

### 15.5 trx-id inversion 교정 + serve 재확인 + ⑥ 재측 (세션 8 이어서)
**serve 재확인 ✅**(`build_d5_serve_rw.sh`): lineage 워크 위 `oltp_read_write`에서 mode-2 verify-serve
`construct_BAD=0·served=hit=643649`, mode-1 serve-only `served=hit=765146`·held-reader SUM 679715 일관 →
일반 OLTP serve correctness 확립.

**⑥ 재측서 회귀 + 근본원인**: 첫 lineage 워크(15.2의 `version<writer` 필터)가 deep held-reader
(`oltp_update_non_index`, hot 행 ~35만 버전)에서 noncontig MISS ~5%를 내고, 그 MISS가 deep undo-walk로
fallback해 64M에서 37.9s·물리 read 39만(이전 0.16s/read 8)으로 회귀. 계측 결과 chase가 깊은 체인서 끊김
(`end=1`=중간 링크 부재·`head==live`·`dropped=0`)인데, 끊긴 predecessor 엔트리는 **존재하나 version>writer**
(`ver_ge_writer=1`, 차이 ~1)라 `version<writer` 필터가 within-trx intermediate(version==writer)로 오인해
양 맵에서 제외 → chase 단절. **원인 = trx_id 발급순서 ≠ commit 순서**: 8스레드 동시성에서 작은 id의 트랜잭션이
큰 id가 만든 값을 나중에 덮어쓰면 pre-image의 DB_TRX_ID(version)가 writer보다 크다. 이런 inverted 링크는 적법한
roll_ptr 링크이므로 버리면 안 된다.

**수정**: 링크 필터를 `version_trx_id < writer_trx_id` → **`version_trx_id != writer_trx_id`**(exact
within-txn intermediate만 제외) + chase의 strict-decrease break 제거(트랜잭션 id 대소가 아니라 **id-equality
링크**[writer → 그 writer가 덮어쓴 version]만 따라가므로 inversion에 견고; roll_ptr chain은 acyclic이라 hop
cap으로 종료) + inversion-위험한 `suffix_min` watermark 게이트 제거(inversion 하에 정당한 boundary version의
id가 run 첫 version보다 작을 수 있어 id-비교 watermark는 잘못된 MISS를 냄; lineage 워크 자체가 boundary가 live
chain 위임을 증명하고 `head_writer==live_top_writer` 전제는 유지).

**검증**: standalone Release 33 green(Consult 9 — MissInteriorDrop 등은 이제 link-absent 경로로 MISS), 통합
`oltp_read_write` construct_BAD=0 유지·HIT율 85.4→88.3%(inversion 링크가 더 많은 체인 연결), **⑥ 재측 4G/64M
...
> ⚠️ **SUPERSEDED (GC-on 이후).** 이 roll_pred back-edge chase는 ⑤(GC on)에서 **GC-unsafe(dangling roll_pred UAF)** 로
> 판정돼 **폐기**됐다 — design-D5-gc §11 NO-GO + 세션 12 재리뷰(7에이전트)가 C3 `gc_generation` 게이트로도 구제 불가임을
> 확정(UAF가 chase 도중 발생하는데 게이트는 HIT 시점 *사후* 검사라 못 막고, 지배적인 prior-cycle free는 탐지조차 못 함).
> **현재 consult는 ⑤b-lite**(헤더에 메모이즈한 live-chain map; GC가 노드 free 전에 cache를 clear → Guard-pinned; reuse는
> node_count+gc_generation으로 self-validate; ~0.22s reuse·~470×)를 쓴다. `roll_pred`/`newest_node`는 insert가 유지하지만
> consult가 **추적하지 않는다**(아래 ②의 "GC 상호작용 재검증 필요"는 그 결과 NO-GO로 종결). 아래 본문은 GC-off 시절의 역사적 기록이다.
