...
**불변식 유지**: consult=LOCATOR/HINT — HIT(=vanilla byte-동일) 또는 MISS(full walk), silent wrong result 구조적 불가. 재현 `build_d5_diag6.sh`(통합 shadow)·`build_d5_walk_std.sh`(standalone)·`build_d5_walk_san.sh`(ASan/TSan, 레포 밖). 커밋 `8a68c79`.

**세션 8 이어서 — serve 재확인 ✅ + ⑥ 재측서 회귀 발견·근본원인·`!=` 수정으로 해소**: ① **serve 재확인 ✅**(`build_d5_serve_rw.sh`): lineage 워크 위 `oltp_read_write` mode-2 verify-serve construct_BAD=0·served=hit=643649, mode-1 serve-only served=hit=765146·held-reader SUM 679715 일관 → 일반 OLTP serve correctness 확립. ② **⑥ 재측서 회귀**: 첫 lineage 워크가 deep held-reader(`oltp_update_non_index`, hot 행 ~35만 버전)서 noncontig MISS ~5%→deep undo-walk fallback→64M 37.9s·물리read 39만. **근본원인(계측, 결정적)**: chase가 깊은 체인서 끊김(`end=1`·`head==live`·`dropped=0`)인데 끊긴 predecessor 엔트리는 **존재하나 version>writer**(차이~1)라 `version<writer` 필터가 within-trx intermediate로 오인·제외 — 원인은 **trx_id 발급순서≠commit순서**(동시성 하 작은 id가 큰 id 값을 나중에 덮어쓰면 pre-image DB_TRX_ID>writer; inversion 링크는 적법). ③ **수정**: 필터 `version<writer`→**`version!=writer`** + chase strict-decrease break 제거(id-equality 링크만 따라감) + inversion-위험한 `suffix_min` 게이트 제거(walk가 cross-gen 배제, `head==live` 전제 유지). ④ **검증**: standalone Release 33 green, `oltp_read_write` construct_BAD=0 유지·HIT율 85.4→88.3%, **⑥ 재측 4G/64M deep-reader HIT 2000/2000·noncontig 0·물리read 0~10(64M cliff 소멸)·latency ~0.36/0.40s 평탄**(D-0 0.8s/123s 대비 큰 승리). **남은 것 = perf만**(절대 latency ~0.4s가 구 ~0.16s보다 높음, per-call link-map 빌드 비용). ⑤ **O(C) pred-pointer 재설계 완료(커밋 `<이번>`)**: 노드에 `roll_pred` 포인터를 insert 때 세팅(단일 드레이너 per-PK=commit/roll_ptr 순서라 "직전 삽입=predecessor")+헤더 `newest_node`/`node_count`. consult가 매-호출 맵 빌드 없이 newest_node→roll_pred 포인터만 따라가 첫 visible 반환=O(depth)·alloc 0. 안전(head full-PK+writer==live, hop마다 id-equality 링크검증→갭 MISS, full-PK 불일치 MISS, bottom NOVISIBLE; 드레이너 append-only·roll_pred 불변→consult immutable 노드만, EBR Guard race 0). **검증: standalone 33+ASan/TSan 22 green, `oltp_read_write` construct_BAD=0(HIT 78%=보수적 MISS·safe), ⑥ 4G/64M deep-reader HIT 2000/2000·물리read 0·latency ~0.16s 평탄(맵 0.4s→0.16s, 원래 payoff 회복; vanilla 0.8s/123s 대비 ~5×/~770×).** = cross-gen+inversion 정확하면서 ⑥ payoff 완전 회복. ⚠️ ⑤ GC 상호작용(roll_pred dangling) 5-2서 재검증 필요. **다음 = ⑤ purge-view GC 5-2.** 재현 `build_d5_serve_rw.sh`·`build_d5_d6_walk.sh`.
