#!/usr/bin/env bash
# Phase 3 gate ① (error bars) for the ⓠ5 effective speedup -- the cache's actual target (a held analytic
# reader) on a write-heavy HTAP profile WITH the delete+reinsert MISS pattern (oltp_write_only churn). The
# held deep read does the same SUM under mode 0 (vanilla walk) vs mode 1 (serve; MISS rows still walk),
# across a BP sweep (4G/256M/64M). GC off -> isolates from the ⑥ chain-sever, so serve does NOT degrade here.
# Re-runs each (BP,mode) N times -> latency median + min/max + a per-BP speedup = median(m0)/median(m1).
# Reports consult HIT% + physical undo reads + construct_BAD. Raw logs + CSV in integration/results/ (gate ②).
# docs/phase2-q3-llt.md ⓠ5. mysqld must be pre-built (current accel sources).
#
# Usage: build_q5_multirun.sh [N]   (default 3).
set -u
N="${1:-3}"
...
CSV="$RESULTS/q5_writeonly.csv"
LOG="$RESULTS/q5_writeonly_run.log"
exec > "$LOG" 2>&1
...
run_one(){  # $1=BP  $2=mode  $3=iter
  ...
  local MLOG="$RESULTS/q5_${tag}_mysqld.log"; local SLOG="$RESULTS/q5_${tag}_scan.log"
  echo ""; echo "########## $tag (N=$N) ##########"
...
SQL
  ...
  local SNAP=$!
  sleep 2
  sysbench oltp_write_only $C --tables=1 --table-size=1000 --threads=8 --time=44 --rand-type=pareto run > "$RESULTS/q5_${tag}_churn.log" 2>&1 &
  local CH=$!
...
echo "=== ⓠ5 effective-speedup MULTI-RUN (oltp_write_only churn + held deep reader, GC off). N=$N ==="
date
...
med(){  # $1=bp $2=mode -> echoes median latency only
  local bp="$1" md="$2"
  local vals=$(awk -F, -v b="$bp" -v m="$md" '$2==b && $3==m && $4!="NA"{print $4}' "$CSV" | sort -n)
...
statline(){
  ...
  local bad=$(awk -F, -v b="$bp" -v m="$md" '$2==b && $3==m{s+=$9} END{print s+0}' "$CSV")
  printf " construct_BAD_total=%s\n" "$bad"
}
echo ""; echo "=== SUMMARY (deep1 latency s; speedup = median(m0)/median(m1)) ==="
for BP in 4G 256M 64M; do
...
echo "construct_BAD must be 0 in every row. (GC off here -> serve does not chain-sever; expect no degrade.)"
echo "=== DONE ==="; date
